...
Xdis <- array(0, dim = c(n, (p + q), R))#(p + p + q +1), R))
trt <- matrix(0, n, R)
...
res <- foreach(k = 1:K) %dorng%
  {
    ...
    fit1 <- rstan::stan(file = "model.stan", data = ss_data1, cores = 1, iter = 100, 
                        chains = 1, verbose = T, warmup = 50, seed = 121,
                        control = list(max_treedepth = 15, adapt_delta = 0.995))#995))
    
    fit2 <- rstan::stan(file = "model.stan", data = ss_data2, cores = 1, iter = 100,
                        chains = 1, verbose = T, warmup = 50, seed = 121,
